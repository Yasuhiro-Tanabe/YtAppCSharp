...
namespace MemorieDeFleursTest.ModelTest
{
    ...
    /// </summary>
    [TestClass]
    public class StockActionLogicTest : MemorieDeFleursTestBase
    {
        ...
        public void CanAddOneOrderToSingleSupplier()
        {
            using(var context = new MemorieDeFleursDbContext(TestDB))
            {
                ...
                var expectedLotNumber = Model.SupplierModel.Order(context, orderDate, ExpectedPart, numLot, arrivalDate);
        ...
        public void CanAddManyOrdersToSingleSupplier()
        {
            using (var context = new MemorieDeFleursDbContext(TestDB))
            {
                ...
                AssertAllLotNumbersAreUnique(context);
        ...
        public void WillBeUsedEarliestArrivedStockLotWhenTwoOrMoreLotHasEnoughQuantity()
        {
            using (var context = new MemorieDeFleursDbContext(TestDB))
            {
                var actualRemain = Model.BouquetModel.UseBouquetPart(context, ExpectedPart, DateConst.May1st, 60);
        ...
        public void CanRemoveOneOrdersOfSingleSupplier()
        {
            using (var context = new MemorieDeFleursDbContext(TestDB))
            {
                ...
                Model.SupplierModel.CancelOrder(context, expectedCanceledLotNumber);
        ...
        /// </summary>
        private void AssertAllLotNumbersAreUnique(MemorieDeFleursDbContext context)
        {
            ...
            foreach (var i in InitialOrders)
            {
                ...
                {
                    var actual = context.StockActions.Count(a => a.Action == StockActionType.SCHEDULED_TO_ARRIVE && a.StockLotNo == j.LotNo);
                    var days = context.StockActions.Where(a => a.Action == StockActionType.SCHEDULED_TO_ARRIVE && a.StockLotNo == j.LotNo).Select(a => a.ArrivalDate);
