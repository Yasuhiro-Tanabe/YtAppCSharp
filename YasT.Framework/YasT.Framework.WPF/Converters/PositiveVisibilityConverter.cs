...
namespace YasT.Framework.WPF.Converters
{
    ...
    {
        /// <summary>
        /// 真理値を WPF UI コンポーネントの画面表示状態に変換する：真のとき表示状態(<see cref="Visibility.Visible"/>)、
        /// 偽のときパラメータで指定された非表示状態 (<see cref="Visibility.Hidden"/> または <see cref="Visibility.Collapsed"/>)。
        /// </summary>
        /// <param name="value">入力値(真理値)。</param>
        /// <param name="targetType">出力データ型。(<see cref="Visibility"/>)</param>
        /// <param name="parameter">非表示状態の指定：
        ///   <list type="bullet">
        ///     <item>"Hidden" のときは value = false だったら <see cref="Visibility.Hidden"/>。</item>
        ///     <item>省略時および "Hidden" 以外のときは value = false だったら <see cref="Visibility.Collapsed"/>。</item>
        ///   </list>
        /// </param>
